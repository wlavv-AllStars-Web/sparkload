...
class Lists extends KlaviyoAPI
{
    ...
    {
        $path = sprintf('%s/%s', self::ENDPOINT_LIST, urlencode($listId));
    ...
    public function addSubscribersToList($listId, $profiles)
    {
        ...
        $path = sprintf('%s/%s/%s', self::ENDPOINT_LIST, urlencode($listId), self::ENDPOINT_SUBSCRIBE);
    ...
    public function addMembersToList( $listId, $profiles )
    {
        ...
        $path = sprintf( '%s/%s/%s', self::ENDPOINT_LIST, urlencode($listId), self::ENDPOINT_MEMBERS );
    ...
    public function getListExclusions($listId, $marker = null)
    {
        ...
        $path = sprintf('%s/%s/%s/%s',self::ENDPOINT_LIST, urlencode($listId), self::ENDPOINT_EXCLUSIONS, self::ENDPOINT_ALL);
    ...
    public function getAllMembers($groupId, $marker = null)
    {
        ...
        $path = sprintf('%s/%s/%s/%s',self::ENDPOINT_GROUP, urlencode($groupId), self::ENDPOINT_MEMBERS, self::ENDPOINT_ALL);
        return $this->v2Request($path, $params);
    }
}
