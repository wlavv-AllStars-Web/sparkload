...
class DataHelper #extends Mage_Core_Helper_Abstract
{
        ...
        if ($this->isEnabledCustomMapping()) {
            ...
            foreach ( $defaultMethods as $default_method ) {
                ...
                if (!empty($mapping) && trim($mapping)!='') {
                    $customMapping[] = \Tools::strtolower( $default_method['id_carrier'] ) . ':' . $mapping;
                }
            }
    ...
    public function getCustomMappingService($shippingMethod)
    {
        if ($this->isEnabledCustomMapping()) {
            $defaultMethods = \Carrier::getCarriers(\Context::getContext()->language->id);
            ...
            $service = array_search( $shippingMethod, $customMapping );
            if (  $service !== false ) {
                return explode(':',$customMapping[$service])[1];
            }
        }
        return '';
    }
